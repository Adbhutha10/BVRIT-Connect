...
interface Conversation {
  ...
  participantRole: 'student' | 'mentor' | 'peer' | 'admin';
  lastMessage: string;
...
const AlumniCommunication: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Fetch mentorships where the user is the mentor
    ...
    const unsubscribe = onSnapshot(mentorshipsQuery, (snapshot) => {
      const convs: Conversation[] = snapshot.docs.map(doc => {
        ...
        return {
          id: doc.id,
          participantName: data.studentName || 'Student',
          participantPhoto: data.studentProfileImage || data.studentPhotoURL,
  ...
  // Fetch participant status (students) for all conversations
  useEffect(() => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await updateDoc(mentorshipRef, {
        lastMessage: `Shared a document: ${file.name}`,
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // 3. Trigger notification for the student
      await addDoc(collection(db, "notifications"), {
        recipientId: selectedChat.participantId,
        content: `New message from mentor: ${newMessage.substring(0, 50)}${newMessage.length > 50 ? '...' : ''}`,
        type: 'message',
        timestamp: serverTimestamp(),
        readAt: null,
      ...
      <div className={`w-full md:w-80 lg:w-96 bg-white border-r border-gray-100 flex flex-col ${selectedChat ? 'hidden md:flex' : 'flex'}`}>
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-bold text-gray-900">Student Chats</h1>
      ...
      <div className={`flex-1 flex flex-col bg-white ${!selectedChat ? 'hidden md:flex' : 'flex'}`}>
        {selectedChat ? (
                    ...
                    <div className={`p-4 rounded-2xl shadow-sm text-sm max-w-[80%] ${
                      ...
                      {msg.fileUrl && (
                        <a 
                          ...
                          className="mt-2 flex items-center p-2 bg-blue-50/50 rounded-lg border border-blue-100 text-blue-700 hover:bg-blue-100 transition-all text-xs"
                        >
                          <FileText className="h-4 w-4 mr-2" /> View Shared Document
                        </a>
                      )}
                    </div>
                  </div>
              ...
              <div className="p-6 bg-white border-t border-gray-100">
                <div className="flex items-center gap-3 bg-gray-50 p-2 rounded-2xl ring-1 ring-gray-200 focus-within:ring-blue-200 transition-all">
                  ...
                  <Button 
                    ...
                    onClick={() => document.getElementById('file-upload-alumni')?.click()}
                    disabled={isUploading}
                  >
            ...
            <div className="hidden lg:flex w-72 bg-gray-50/50 border-l border-gray-100 flex-col p-6">
              <div className="flex flex-col items-center text-center mb-8">
                <div className="h-24 w-24 rounded-3xl bg-white p-1 shadow-md mb-4 ring-1 ring-gray-100">
                  <div className="h-full w-full rounded-2xl bg-blue-100 overflow-hidden">
                    {selectedChat.participantPhoto ? <img src={selectedChat.participantPhoto} className="h-full w-full object-cover" /> : <User className="h-12 w-12 text-blue-400 mt-6 ml-6" />}
                  </div>
                </div>
                <h3 className="font-bold text-lg text-gray-900">{selectedChat.participantName}</h3>
                <Badge className="mt-2 bg-blue-50 text-blue-600 border-none capitalize">{selectedChat.participantRole}</Badge>
              </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-10">
            ...
            <h2 className="text-xl font-bold text-gray-900">Student Conversations</h2>
            <p className="text-gray-500 mt-2 max-w-xs">
              Select a student from the left to view messages and provide guidance.
            </p>
          </div>
        )}
      </div>
